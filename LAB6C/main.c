...
void SetupEDMA3( Uint32 edma_evt, int i, int j )
{
    ...
    edma3ccRegs->PARAMSET[edma_evt].DST = (Uint32)image + i*PIXELS_IN_IMAGE/3 + j*PIXELS_PER_LINE/3 + i*PIXELS_IN_IMAGE/6 + 128;
...
void SetupEDMA3Down( Uint32 edma_evt, int i, int j )
{

    edma3ccRegs->PARAMSET[edma_evt].SRC = (Uint32)image;
    edma3ccRegs->PARAMSET[edma_evt].DST = (Uint32)image + i*PIXELS_IN_IMAGE/6 + j*PIXELS_PER_LINE/6;
...
void SetupEDMA3Up( Int32 ch, Int32 offset )
{
    ...
    edma3ccRegs->PARAMSET[ch].DST = (Uint32)image + PIXELS_IN_IMAGE/3 + PIXELS_PER_LINE/3 + offset;
    ...
    edma3ccRegs->PARAMSET[ch].A_B_CNT =           CSL_FMK(TPCC_PARAM_A_B_CNT_BCNT, X_SIZE)
                                                    | CSL_FMK(TPCC_PARAM_A_B_CNT_ACNT, 1);
    edma3ccRegs->PARAMSET[ch].SRC_DST_BIDX =    CSL_FMK(TPCC_PARAM_SRC_DST_BIDX_DSTBIDX, 2)
    ...
    edma3ccRegs->PARAMSET[ch].SRC_DST_CIDX =    CSL_FMK(TPCC_PARAM_SRC_DST_CIDX_DSTCIDX, PIXELS_PER_LINE*2)
...
void Task_edma( void )
{
    ...
    for( int i=2; i<6; i++)
        ...
        {
            ResetEDMA3( );
            SetupEDMA3Down( EDMA_EVENT5, i, j );
    ...
    ResetEDMA3( );
    SetupEDMA3Up(EDMA_EVENT5, 1 );
    CSL_FINS(edma3ccRegs->TPCC_ESR, TPCC_TPCC_ESR_E5, 1);
    while( (edma3ccRegs->TPCC_IPR & (1<< EDMA_EVENT5)) == 0 );

    ResetEDMA3( );
    SetupEDMA3Up(EDMA_EVENT5, PIXELS_PER_LINE );
    CSL_FINS(edma3ccRegs->TPCC_ESR, TPCC_TPCC_ESR_E5, 1);
    while( (edma3ccRegs->TPCC_IPR & (1<< EDMA_EVENT5)) == 0 );

    ResetEDMA3( );
    SetupEDMA3Up(EDMA_EVENT5, PIXELS_PER_LINE+1 );
    CSL_FINS(edma3ccRegs->TPCC_ESR, TPCC_TPCC_ESR_E5, 1);
    while( (edma3ccRegs->TPCC_IPR & (1<< EDMA_EVENT5)) == 0 );
